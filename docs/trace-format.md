...
dependency-free codec (`src/format/trace.hpp`), replays trace layers during
image bring-up, and records live-device traces through the supervisor
protocol. The governing decision is the accepted
[ADR-0013](./adr/0013-trace-prefetch.md) (record/replay prefetch traces in
the upstream format with a dependency-free codec; this document was its
