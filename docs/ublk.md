...
| `DISCARD` | If writable root: `WritableBlobSource::discard()`; writable uppers provide ADR-0009 mask-with-zeroes semantics. LSMT-RW records zeroed segments, while sparse uppers punch holes and persist a dirty sidecar zero map at FLUSH/checkpoint. Success result = 0. Otherwise **-EROFS**. |
