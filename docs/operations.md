...
| `discard`/`fstrim` fails with EROFS | The image is read-only (no writable upper configured). Discard reaches only writable devices; writable uppers satisfy ADR-0009 zero-mask semantics. LSMT-RW records zeroed segments, while sparse uppers punch holes and persist a dirty sidecar zero map at flush/checkpoint. |
...
- **LSMT-RW discard masks, it does not punch through.** A discarded range
  ...
  deallocation with sidecar zero-mask metadata persisted at flush/checkpoint.
