...
pure library, but the repository still lacked the concrete converter boundary
and a dependency-free path that can run in the default build and CI.
...
ustar archive from a file or stdin, spools file payloads to disk while keeping
only metadata in memory, builds a deterministic ext2-compatible filesystem image
with the built-in backend, seals that raw image as a single LSMT-RO layer, and
prints manifest metadata (`digest`, `size`, `file`, and converter fields) on
stdout.

The built-in backend is the default no-extra-dependency backend. It must not
invoke host `mkfs`, open a block device, or mount anything. All filesystem
fields that would otherwise vary by host or time are pinned: timestamps are
