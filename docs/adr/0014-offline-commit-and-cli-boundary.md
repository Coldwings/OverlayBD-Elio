...
- **The streaming converter lives in this repository**, beside the
  format writers it must share. ADR-0019 now accepts the pinned
  e2fsprogs/libe2fs backend as the default enabled converter backend, while the
  dependency-free built-in ext2 backend remains available behind the same binary
  and covered by CI. The external CLI repository owns everything that merely
  talks to a registry: reference resolution UX,
