// CRC-32C implementation. See crc32c.hpp for the exact semantics contract.
//
// The hardware path and the table are derived from the Apache-2.0 crc32c
// code vendored in containerd/overlaybd (src/overlaybd/zfile/crc32/), which
// itself credits Facebook (folly) and Intel. Kept byte-compatible on
// purpose: ZFile digests must match bit for bit.
#include "common/crc32c.hpp"

#include <algorithm>
#include <cstdint>

// Hardware CRC-32C availability. The crc32 intrinsics are compiler builtins
// (__builtin_ia32_crc32* / __builtin_aarch64_crc32c*); on GCC the x86 ones
// still require the intrinsic header for their declarations. Runtime
// detection goes through __builtin_cpu_supports on x86.
#if defined(__x86_64__) || defined(__i386__)
#include <immintrin.h>
#define OBD_CRC32C_X86 1
#elif defined(__aarch64__) && defined(__ARM_FEATURE_CRC32)
#define OBD_CRC32C_ARM 1
#endif

namespace obd::crc32 {
namespace {

// Rocksoft-model table: poly 0x1EDC6F41, reflected, slice-by-1.
...
#if defined(OBD_CRC32C_X86)
// The crc32 builtins require the sse4.2 ISA at the function level; runtime
// dispatch below guards execution with __builtin_cpu_supports("sse4.2").
...
uint32_t crc32c_hw(const uint8_t* data, size_t nbytes, uint32_t crc) {
    ...
    size_t offset = 0;
#if defined(OBD_CRC32C_X86)
    // Align to 8 bytes, then consume 8 bytes per instruction.
    size_t mask = static_cast<size_t>(reinterpret_cast<uintptr_t>(data) & 7u);
    if (mask != 0) {
        size_t limit = std::min(nbytes, sizeof(uint64_t) - mask);
        while (offset < limit) {
            sum = static_cast<uint32_t>(__builtin_ia32_crc32qi(sum, data[offset]));
            offset++;
        }
    }
    while (offset + sizeof(uint64_t) <= nbytes) {
        uint64_t word;
        __builtin_memcpy(&word, data + offset, sizeof(word));
        sum = static_cast<uint32_t>(__builtin_ia32_crc32di(sum, word));
...
#elif defined(OBD_CRC32C_ARM)
    while (offset + sizeof(uint64_t) <= nbytes) {
        uint64_t word;
        __builtin_memcpy(&word, data + offset, sizeof(word));
        sum = static_cast<uint32_t>(__builtin_aarch64_crc32cx(sum, word));
        offset += sizeof(uint64_t);
    }
    while (offset < nbytes) {
        sum = static_cast<uint32_t>(__builtin_aarch64_crc32cb(sum, data[offset]));
        offset++;
    }
#endif
    return sum;
...
uint32_t crc32c_sw(const void* data, size_t nbytes, uint32_t crc) {
    ...
    while (nbytes--) {
        crc = kCrc32cTable[(crc ^ *buf++) & 0xff] ^ (crc >> 8);
    }
    return crc;
}
...
uint32_t crc32c_extend(const void* data, size_t nbytes, uint32_t crc) {
    ...
    if (hardware_available()) {
        return crc32c_hw(static_cast<const uint8_t*>(data), nbytes, crc);
    }
    return crc32c_sw(data, nbytes, crc);
}
